#!/bin/bash
SCRIPT_PATH='/data5/bio/runs-galkin/prjeb_agp_comb_counts/Scripts'
SPARCC_PATH='/data5/bio/runs-galkin/prjeb_agp_comb_counts/SparCC_source'
...
if [ "$Tt" -eq 0 ]; then
	...
	if [ ! -f $out_SparCC/cor_mat_SparCC.out ]; then
		...
		num_resamps=$(ls $out_SparCC/Resamplings/ | wc -l)

		if [ $num_resamps -lt $BOOT_ITER ]; then
	...
	if [ $num_straps -lt $BOOT_ITER ]; then
		...
		declare -A hostnames
		hostnames+=( [0]="node1.net0.pyxis.ripcm.com" [1]="node2.net0.pyxis.ripcm.com" [2]="node4.net0.pyxis.ripcm.com" [3]="node5.net0.pyxis.ripcm.com" [4]="node11.net0.pyxis.ripcm.com" [5]="node12.net0.pyxis.ripcm.com" )
		...
		do
			...
			do
				...
				if [ ! -f $out_SparCC/Bootstraps/sim_cor_$taskno.txt ]; then
					tasknoW=$(($j*6+$i-6))
					node=${hostnames[$i]}
					echo "python $SPARCC_PATH/SparCC.py $out_SparCC/Resamplings/resamp_$taskno.txt -i $ITER --cor_file=$out_SparCC/Bootstraps/sim_cor_$taskno.txt >> $out_SparCC/sparcc.log" | qsub -pe make 3 -N J${job_prefix}_${taskno} -hold_jid J${job_prefix}_${tasknoW} -cwd -l hostname=$node -r yes
				fi
			done
		done
	fi
	...
	if [ $boot_samples -lt $BOOT_ITER ]; then
		...
		reserved_hostnames+=( [0]="node6.net0.pyxis.ripcm.com" [1]="node8.net0.pyxis.ripcm.com" [2]="node2.net0.pyxis.ripcm.com" [3]="node12.net0.pyxis.ripcm.com")
