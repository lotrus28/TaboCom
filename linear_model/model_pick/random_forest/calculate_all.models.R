...
p_counts = args[2]
p_tax_code = args[3]
assign("cor_thr", as.numeric(args[4]), envir = .GlobalEnv)
out = args[5]
...
get_pair_models_table = function(path_to_cor = NA, path_to_counts = NA, 
  ...
  produce_pair_models <- function(sig_pairs, initial_data, pV_cutoff, num_boot) {
    ...
    all_models = matrix(list(NA),
                        ncol = length(unique(sig_pairs)),
                        nrow = length(unique(sig_pairs))
                        )
    
    rownames(all_models) <- unique(sig_pairs)
    colnames(all_models) <- unique(sig_pairs)
      ...
      for (i in 1:L) 
    {
        ...
        for (k in c('direct', 'reverse')) {
          ...
          mysteriousError = tryCatch({
			length(temp[,colSums(temp^2) == 0]) > 0
		},
		 error = function(err) {
			err
			})

	  if(inherits(mysteriousError, "error")) {
		next
	  } else {
		  if(mysteriousError) next
		}
...
# Removed bootstrap pvalue calculation
# It slows everything down really much
# Plus it might work improperly
temp = get_pair_models_table(cor_table = sig.cor, counts = counts, bootnum = 1, boot_pV = 1)
pair_mod = temp$mods
pvals = temp$pvs
